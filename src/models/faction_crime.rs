/*
 * Torn API
 *
 *   * The development of Torn's API v2 is still ongoing.  * If selections remain unaltered, they will default to the API v1 version.  * Unlike API v1, API v2 accepts both selections and IDs as path and query parameters.  * If any discrepancies or errors are found, please submit a [bug report](https://www.torn.com/forums.php#/p=forums&f=19&b=0&a=0) on the Torn Forums.
 *
 * The version of the OpenAPI document: 2.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct FactionCrime {
    #[serde(rename = "id", skip_serializing_if = "Option::is_none")]
    pub id: Option<i32>,
    #[serde(rename = "name", skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    #[serde(rename = "difficulty", skip_serializing_if = "Option::is_none")]
    pub difficulty: Option<i32>,
    #[serde(rename = "status", skip_serializing_if = "Option::is_none")]
    pub status: Option<models::FactionCrimeStatusEnum>,
    /// The timestamp at which the crime was created.
    #[serde(rename = "created_at", skip_serializing_if = "Option::is_none")]
    pub created_at: Option<i32>,
    /// Replaced by the 'planning_at' field (same value). This field will be removed in the future (early March or later).
    #[serde(rename = "initiated_at", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub initiated_at: Option<Option<i32>>,
    /// The timestamp at which the planning phase for the crime has begun.
    #[serde(rename = "planning_at", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub planning_at: Option<Option<i32>>,
    /// The timestamp at which the crime will be ready.
    #[serde(rename = "ready_at", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub ready_at: Option<Option<i32>>,
    /// The timestamp at which the crime will expire.
    #[serde(rename = "expired_at", skip_serializing_if = "Option::is_none")]
    pub expired_at: Option<i32>,
    #[serde(rename = "slots", skip_serializing_if = "Option::is_none")]
    pub slots: Option<Vec<models::FactionCrimeSlot>>,
    #[serde(rename = "rewards", skip_serializing_if = "Option::is_none")]
    pub rewards: Option<Box<models::FactionCrimeRewards>>,
}

impl FactionCrime {
    pub fn new() -> FactionCrime {
        FactionCrime {
            id: None,
            name: None,
            difficulty: None,
            status: None,
            created_at: None,
            initiated_at: None,
            planning_at: None,
            ready_at: None,
            expired_at: None,
            slots: None,
            rewards: None,
        }
    }
}

